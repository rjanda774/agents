...
class Me:
    ...
    def chat(self, message, history):
        # Gradio ChatInterface(type="messages") gives history like:
        # [{"role":"user","content":"..."}, {"role":"assistant","content":"..."}...]
        input_messages = [{"role": "system", "content": self.system_prompt()}] + history + [
        ...
        # 2) If tools requested, run them and continue the same response chain
        while tool_calls:
            tool_outputs = self.handle_tool_call(tool_calls)

            resp = self.client.responses.create(
                model="gpt-4o-mini",
                # Continue the same conversation + provide tool outputs.
                # We append the assistant output from prior response by passing `previous_response_id`
                # and sending tool outputs.
