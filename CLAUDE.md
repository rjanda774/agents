...
- **Hard-enforced rules**: 25–45 day expirations only (rejected server-side in `sell_credit_spread`, not just by prompt), short-leg delta 0.20–0.30, minimum premium and probability-of-profit thresholds, mandatory position-management checks (take profit at 75% captured, cut losses if the short strike is breached, force-close inside 7 days-to-expiration to avoid assignment/pin risk) — all spelled out at length in `templates.py`'s Cathie-specific `trader_instructions`/`trade_message`/`rebalance_message`, since the LLM is trusted to follow the workflow, not just given tools and hoped for the best.
